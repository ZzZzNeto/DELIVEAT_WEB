...
export default function CardOrder({index,id,name, date, price, address, user} : data){
        ...
        <Draggable key={id} draggableId={`${id}`} index={index}>
            {(draggableProvided, draggableSnapshot) => (
                <div ref={draggableProvided.innerRef} {...draggableProvided.draggableProps} {...draggableProvided.dragHandleProps}  className=" w-[233px] mb-[20px] h-[122px] rounded-[10px] bg-bg px-[17px] py-[10px]">
                        ...
                        >
                            <OrderModal code='K23UR3' buttons close={closeModal} items={items} user={{name:"Rodolfo Padilha"}} addres={{street: "Rua das flores", number:158, city:"Pau dos Ferros", country:"RN"}} total={50.00} cupom={50} final={25.00} method='Cartão de credito'/>
                        </Modal>
                    ...
                    <div className='flex items-center mt-[3px]'>        
                        ...
                        <p className='text-[12px] ml-[5px]'>{user}{id}</p>
                    </div>
